...
const refs = {
  ...
  warningNote: document.querySelector("#warning-note"),
...
function isValidSpread(waccRate, terminalRate) {
  return Number.isFinite(waccRate) && Number.isFinite(terminalRate) && waccRate - terminalRate > MIN_SPREAD;
}

function computeIntrinsicValue({ wacc, terminalGrowth, fcfShift }) {
  const waccRate = wacc / 100;
  const terminalRate = terminalGrowth / 100;
  const growthRates = WORKBOOK_DEFAULTS.baseGrowthRates.map((rate) => (rate + fcfShift) / 100);

  if (!isValidSpread(waccRate, terminalRate) || state.sharesOutstanding <= 0) {
    return Number.NaN;
  }
  ...
  const terminalValue = (projectedFcf * (1 + terminalRate)) / (waccRate - terminalRate);
...
function buildScenarioDetails(label, scenario, intrinsicValue) {
  return [
    ...
    [
      "FCF growth shift",
      `${scenario.fcfShift >= 0 ? "+" : ""}${scenario.fcfShift.toFixed(2)}%`,
    ],
    ["Net debt", formatCurrency(state.netDebt)],
    [
      "Shares outstanding",
      new Intl.NumberFormat("en-US", { maximumFractionDigits: 0 }).format(state.sharesOutstanding),
    ],
    [
...
function renderBriefExplanation() {
  ...
  const paragraph = `The model shows that ${strongest.label} is the most significant determinant of intrinsic value, producing an estimated per-share change of ${formatCurrency(strongest.magnitude)} when adjusted by +/-2%. ${middle.label} has a secondary impact, while ${weakest.label} is relatively less influential. This pattern suggests that valuation is highly sensitive to discount rate and terminal growth assumptions, reinforcing that small changes in long-term inputs can materially alter outcomes. As a result, the estimated intrinsic value should be interpreted as a valuation range rather than a precise target. ${directionNote}`;
...
function renderActiveHeatmap() {
  ...
  const finiteValues = scenarios.map((entry) => entry.value).filter((value) => Number.isFinite(value));
  ...
  const tableBody = DELTAS.map((yDelta) => {
    const rowCells = DELTAS.map((xDelta) => {
      ...
      const cellData = scenarios.find((entry) => entry.xDelta === xDelta && entry.yDelta === yDelta);
...
function renderSelectedScenario() {
  ...
  const scenario = config.getScenario(Number.isFinite(xDelta) ? xDelta : 0, Number.isFinite(yDelta) ? yDelta : 0);
  const intrinsicValue = computeIntrinsicValue(scenario);
  const details = buildScenarioDetails(
    `${config.title} (${formatPercent(Number.isFinite(xDelta) ? xDelta : 0)}, ${formatPercent(Number.isFinite(yDelta) ? yDelta : 0)})`,
    scenario,
    intrinsicValue,
  );
...
function updateWarnings() {
  const spread = state.baseWacc / 100 - state.baseTerminalGrowth / 100;
  if (spread <= MIN_SPREAD) {
    refs.warningNote.textContent =
      "Warning: WACC is too close to terminal growth. Values can become unstable or undefined.";
    return;
  }
...
function getNumericValue(input, fallback) {
  const parsed = Number(input.value);
  return Number.isFinite(parsed) ? parsed : fallback;
}

function handleInputChange() {
  state.baseWacc = clamp(getNumericValue(refs.waccInput, state.baseWacc), 1, 30);
  state.baseTerminalGrowth = clamp(
    getNumericValue(refs.terminalGrowthInput, state.baseTerminalGrowth),
    -5,
    20,
  );
  state.fcfShift = clamp(getNumericValue(refs.fcfShiftInput, state.fcfShift), -10, 10);
  state.netDebt = clamp(getNumericValue(refs.netDebtInput, state.netDebt), 0, 1_000_000_000_000);
  state.sharesOutstanding = clamp(getNumericValue(refs.sharesInput, state.sharesOutstanding), 1, 100_000_000_000);
  render();
}

function handleSliderChange() {
  state.baseWacc = clamp(getNumericValue(refs.waccSlider, state.baseWacc), 1, 30);
  state.baseTerminalGrowth = clamp(
    getNumericValue(refs.terminalGrowthSlider, state.baseTerminalGrowth),
    -5,
    20,
  );
  state.fcfShift = clamp(getNumericValue(refs.fcfShiftSlider, state.fcfShift), -10, 10);
  render();
}
